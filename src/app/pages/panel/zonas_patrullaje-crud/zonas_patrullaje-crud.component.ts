...
@Component({
  selector: 'app-zonas-patrullaje-crud',
  imports: [ReactiveFormsModule],
  templateUrl: './zonas_patrullaje-crud.component.html',
})


export default class ZonasPatrullajeCrudComponent implements OnInit {
  // Variables
  formUtils = FormUtils;
  ...
  obtenerZonas() {
    ...
    this._zonaService.obtenerZonas().subscribe({
      next: (res) => {
        this.zonas = res; // Guardar lAS ZONAS
      },
      error: (err) => {
  ...
  guardarZona(): void {
    ...
    const zona: ZonaPatrullaje = {
      id: '',
      name: this.zonaForm.value.nombre,
      description: this.zonaForm.value.descripcion,
  ...
  toggleZona(zona: ZonaPatrullaje) {
    ...
    if (visible) {
      ...
      zona.coordinates.forEach(coord => {
